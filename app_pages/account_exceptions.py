...
with tab_edit:
    ...
    # SMS: Filter probably isn't needed for this context, but for consitency in downstream logic I'm 
    # keeping the object in place
    #  ** See also removed Key in data frame object below
    # ----------------------------------------
    ...
    if st.button("Save Changes", key="save_edits", type="primary"):
        ...
        for idx in range(len(filtered_df)):
            ...
            if not orig.equals(curr_comparable):
                ...
                changes.append({
                    "ID": orig["ID"],
                    "GL Account Number": curr["GL Account Number"],
                    "Ledger": curr["Ledger"],
                    "Functional Area": curr["Functional Area"],
                    "Company Code": curr["Company Code"],
                    "Profit Center": curr["Profit Center"],
                    "Cost Center": curr["Cost Center"],
...
with tab_add:
    ...
    with col1:
        new_gl = st.text_input("GL Number *", key="add_gl")
    with col2:
        new_company = st.text_input("Company Code *", key="add_company")
    with col3:
        new_ledger = st.text_input("Ledger *", value="0L", key="add_ledger")
    with col4:
        new_fa = st.text_input("Functional Area *", value="0000", key="add_fa")
    with col5:
        new_pc = st.text_input("Profit Center", key="add_pc")
    with col6:
        new_cc = st.text_input("Cost Center", key="add_cc")
        

    if st.button("Add Mapping", type="primary"):
        missing = []
        if not new_gl.strip():
            missing.append("GL Number")
        if not new_company.strip():
            missing.append("Company Code")
        if not new_ledger.strip():
            missing.append("Ledger")
        if not new_fa.strip():
            missing.append("Functional Area")
        if missing:
            st.error(f"Please fill in required fields: {', '.join(missing)}")
        elif ((master_df["GL Account Number"] == new_gl.strip()) &
              (master_df["Company Code"] == new_company.strip()) &
              (master_df["Ledger"] == new_ledger.strip()) &
              (master_df["Functional Area"] == new_fa.strip())).any():
            st.error("A rule with this GL Number, Company Code, Ledger, and Functional Area already exists.")
        else:
            ...
            with st.spinner("Adding rule..."):
                write_events([evt])
                st.cache_data.clear()
            for k in ["add_gl", "add_company", "add_ledger", "add_fa", "add_pc", "add_cc"]:
